...
const {
  ...
  ForbiddenError,
  NotFoundError,
} = require("../services/errors/BusinessError");
...
module.exports = {
  ...
  read: async (req, res) => {
    try {
      ...
      if (result === null)
        throw new NotFoundError(`Group not found: 존재하지 않는 그룹입니다.`);
  ...
  create: async (req, res) => {
    try {
      ...
      let body = req.body;
      body.admins =
        req.body.admins.length > 0 ? req.body.admins : [res.locals._id];
  ...
  update: async (req, res) => {
    try {
      ...
      // Admin check
      if (!group.admins.some((admin) => admin.equals(res.locals._id)))
        throw new ForbiddenError(
          `Access denied: 그룹의 관리자만 수정할 수 있습니다.`
        );
