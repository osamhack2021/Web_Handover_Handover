...
export const getItem = (itemId) =>
  request.get(`/api/item/${itemId}`).then(handleSuccess).catch(handleError);
...
export const getItemChildren = (itemPath) =>
  request
    .get("/api/item")
    .query({ path: itemPath })
    .then(handleSuccess)
...
export const getRecommendItem = (userId) =>
  request
    .get("/api/recommend")
    .query({ userId: userId })
    .then(handleSuccess)
...
export const algoliaSearch = (query) =>
  request
    .get(`/api/item/algolia/${query}`)
    // .query({ query })
    .then(handleSuccess)
    .catch(handleError);

// update item with item id
export const updateItem = (itemId, object) =>
  request
    .put(`/api/item/${itemId}`)
    .send(object)
    .then(handleSuccess)
...
// update item with item id
export const addItemComment = (itemId, comments) =>
  request
    .put(`/api/item/${itemId}`)
...
export const deleteItem = (itemId) =>
  request.delete(`/api/item/${itemId}`).then(handleSuccess).catch(handleError);

// create item with object
export const createItem = (object) =>
  request.post("/api/item").send(object).then(handleSuccess).catch(handleError);
