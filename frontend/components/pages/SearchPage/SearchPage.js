...
export default function SearchPage() {
  const [searchReusult, setSearchResult] = useState({ cabinet: [], document: [], card: [] });
  const [loading, setLoading] = useState(true);
  const { searchQuery } = useParams();
  useEffect(() => {
    algoliaSearch(searchQuery).then((data) => {
      console.log(`searched with query : ${searchQuery} and response was : ${JSON.stringify(data)}`);
      if (data !== null) {
        setSearchResult({
        ...
        <div className="recommend-block">
          ...
          <hr />
          {
            (searchReusult.cabinet === [])
              ? '검색 결과가 없습니다.'
              : <HorizontalContent type="cabinet" cardArray={searchReusult.cabinet} />
          }
        ...
        <div className="recommend-block">
          ...
          <hr />
          {
            (searchReusult.doucment === [])
              ? '검색 결과가 없습니다.'
              : <HorizontalContent type="doucment" cardArray={searchReusult.document} />
          }
        </div>
        <div className="recommend-block">
